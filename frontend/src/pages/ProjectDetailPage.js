...
export default function ProjectDetailPage() {
  ...
  const isCollaborator =
    user && project && project.collaborators?.some((c) => c._id === user._id);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
      ...
      <Link
        to="/"
        className="inline-flex items-center gap-1.5 text-ink-400 hover:text-white text-sm mb-6 transition-colors"
      >
        <svg
          className="w-4 h-4"
          fill="none"
        ...
        <div className="card mb-5">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
            <div className="flex flex-wrap gap-2">
              <span className="badge bg-ink-700 text-ink-300 border border-ink-600">
                {project.department}
              </span>
              <span
                className={`badge ${project.status === "published" ? "badge-published" : "badge-ongoing"}`}
              >
                {project.status}
              </span>
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-ink-800">
            ...
            <div>
              ...
              <p className="text-xs text-ink-500">
                ...
                {project.ownerId?.department &&
                  `· ${project.ownerId.department}`}
              </p>
            </div>
            <span className="ml-auto badge bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs">
              Owner
            </span>
          </div>
        </div>
            ...
            {project.fileUrl ? (
              <div className="card">
                ...
                <div className="flex items-center gap-3 p-3 bg-ink-800 rounded-lg border border-ink-700">
                  ...
                  <button
                    onClick={() => {
                      // ✅ For Cloudinary raw PDFs - just open the URL directly
                      // The URL already ends with the filename, just open it
                      window.open(
                        project.fileUrl,
                        "_blank",
                        "noopener,noreferrer",
                ...
                <ul className="space-y-3">
                  {project.contributors.map((c) => (
                    <li key={c._id} className="flex items-center gap-2.5">
                      ...
                      <span
                        className={`badge border text-xs ${ROLE_COLORS[c.role] || ROLE_COLORS.viewer}`}
                      >
                        {c.role}
                      </span>
                    </li>
            ...
            <div className="card">
              ...
              <dl className="space-y-2.5 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="text-ink-200 font-medium">
                    {project.department}
                  </dd>
                </div>
                <div className="flex justify-between">
